...
(function() {
  var Promise, _, browserify, buildPages, buildScripts, buildStyles, compileTemplate, compileTemplates, copyFontAwesome, copyImages, exec, fs, glob, log, makeBundle, mkdirp, moment, path, rimraf, rsvp, signalDone, signalError, startBuild, uglifyify;
  ...
  compileTemplates = function(ctx) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      _.each(sources, function(template) {
        var ref;
        if (fs.statSync(template).mtime > (((ref = cache[template]) != null ? ref.mtime : void 0) || 0)) {
      ...
      jstSrc = _.reduce(cache, function(out, arg, file) {
        var js, key;
        js = arg.js;
  ...
  makeBundle = function(ctx) {
    ...
    try {
      ...
      e = error;
      return log.debug("Error preparing bundle: " + e);
    }
  };

  buildScripts = function(ctx) {
    var bundle, startedAt;
    startedAt = moment();
    bundle = makeBundle(ctx);
    return new Promise(function(done, fail) {
      if (bundle) {
        bundle.bundle().pipe(fs.createWriteStream(ctx.args.buildDir + "/js/all.js"));
        return bundle.on('time', function(ms) {
          log.debug("Finished compiling scripts in " + ms + "ms");
          return done();
        }).on('error', function(err) {
          return fail("Error compiling scripts: " + err);
        });
      } else {
        return fail("Error preparing bundle");
      }
    });
  };

  buildStyles = function(ctx) {
    return (require("./css_preprocessors/" + ctx.args.cssPreprocessor))(ctx);
  };
