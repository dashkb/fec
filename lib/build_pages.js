...
(function() {
  var Promise, _, addGitMetadata, buildPages, dateFormat, exec, extractFrontMatter, fm, fs, glob, hamlc, highlight, log, marked, moment, path, renderPages, rsvp, slugify;
  ...
  extractFrontMatter = function(ctx) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      ctx.pageMetadata = _.reduce(files, function(data, file) {
        var attributes, body, dest, ref, relativePath, slug;
        relativePath = path.relative(ctx.args.srcDir, file);
        ref = fm(fs.readFileSync(file, {
          encoding: 'utf8'
        })), attributes = ref.attributes, body = ref.body;
        ...
        return new Promise(function(resolve, reject) {
          return exec("git log --follow --pretty=\"format:%h|%ai\" " + file, function(err, output) {
            ...
            md.revisions = _.map(output.split("\n"), function(line) {
              var date, ref, sha;
              ref = line.split('|'), sha = ref[0], date = ref[1];
              return {
                sha: sha,
                date: date
              };
